...
def _normalize_feishu_group_policy(raw: str | None) -> str:
    """Normalize group policy string to one of: open, mention, managed, managed_or_mention."""
    if not raw:
        return "open"
    ...
    aliases: dict[str, str] = {
        ...
        "at": "mention",
        "at_mention": "mention",
        "default": "managed_or_mention",
    }
    return aliases.get(val, val)


def _message_mentions_bot(message: InboundMessage) -> bool:
    """Return True if message metadata indicates the bot was mentioned."""
    return bool(message.metadata.get("mentions_bot"))
