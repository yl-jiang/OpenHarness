...
示例 2: 家庭成员的健康事件
用户消息: "今天带小明去新华医院做发育评估，Gesell结果正常"
    │
    ▼
Solo Agent (LLM)
    │
    ├─① solo_record(date=今天, tags="家庭,小明,健康,医院", ...)
    │     → 写入 records 表（日常日志）
    │
    └─② solo_health_record(
            subject="小明",          ← 关键：识别出是孩子的健康事件
            category="medical",
            item="Gesell发育评估",
            description="评估结果：正常",
            status="resolved",
...
用户消息: "今天带小明去游乐场玩了，他很开心，小红没有去，因为她去体检了"
    │
    ▼
Solo Agent (LLM)
    │
    ├─① solo_record(date=今天, tags="家庭,小明,游乐场,小红", ...)
    │     → 写入 records 表（日常日志，主要记录游乐场）
    │
    └─② solo_health_record(
            subject="小红",          ← 关键：从附带信息中识别出健康事件
            category="medical",
            item="体检",
            description="小红去体检",
            date=今天
...
CREATE TABLE IF NOT EXISTS health_records (
    ...
    -- 健康记录的主体：'self'（用户自己）或家庭成员名称（如'小明'、'小红'）
...
| `subject` | TEXT | 健康记录的主体：`'self'`（用户自己）或家庭成员名称（如`'小明'`、`'小红'`）。用于区分用户本人和家人（尤其是子女）的健康数据 |
...
def health_record_subjects(self) -> dict[str, int]:
    """返回 {subject: count} 字典，如 {'self': 31, '小明': 13, '小红': 2}。"""
```
...
> `subject` 过滤必须下推到 SQL（走 `idx_health_records_subject` 索引），而不是在 service 层拉全量再 Counter，否则在数据量增长后 `/health?subject=小明` 会变成全表扫描后内存过滤。
...
- 默认 `'self'`（小写固定），表示用户本人。
- 家庭成员名称使用用户在消息中的原始称呼（如"小明"、"小红"、"老妈"），**不做别名归一化**（"老妈"和"妈妈"视为不同 subject，由用户自己在前端合并或重命名，见后续演进）。
...
为 agent 提供查询健康历史的能力（用于回答用户的健康相关问题）。**必须支持 `subject` 参数**，否则 agent 无法回答"小明最近吃药情况"这类多主体问题：
...
note in a daily record (e.g. "小红没去游乐场，因为她去体检了" → extract 小红's medical visit).
...
set subject to their name (e.g. "小明", "小红").
```
...
│  │  2025-06-17 👶小明 medical/Gesell发育评估, 正常     │   │
...
GET  /api/solo/health/subjects
     → 返回所有出现过的 subject 列表及各自记录数（如 {"self": 31, "小明": 13, "小红": 2}）
...
GET  /api/solo/health/records?subject=&category=&status=&date_from=&date_to=&limit=&offset=
     → 分页查询健康记录（subject 过滤下推 SQL：subject=self 只看自己，subject=小明 只看孩子）
...
├── components/
│   └── health/
│       ├── SubjectFilter.tsx            # 主体选择器（全部/自己/小明/小红...）
...
Health.tsx
  │
  ├─ mount → api.health.subjects()            ← 拿到主体列表 [全部/自己/小明/小红]
...
#### v1.2
- **AI 健康洞察**：agent 定期分析健康记录，主动发现模式（如"你最近两周睡眠明显变差"、"小明这个月去医院次数比上月多"）
...
- **儿童成长曲线**：基于 subject=小明 的 vital 类记录，绘制身高/体重/发育里程碑成长曲线
