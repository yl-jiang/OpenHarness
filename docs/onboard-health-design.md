...
| `record_id` | TEXT | 关联的 `records` 表 ID（可空，见 §2.6 关联策略） |
...
#### record_id 关联（重要：避免"有去无回"）

设计初稿曾把"与 records 表可关联"作为选 SoloStore 新表的核心理由，但 `solo_health_record` 是独立工具调用，LLM 不会知道同轮 `solo_record` 的返回 id，**强行让 LLM 传 record_id 注定失效**。因此采用如下明确策略：

1. **LLM 不传 record_id**：`solo_health_record` 的参数列表里**不含** `record_id`（避免幻觉/乱填）。
2. **系统侧尽力关联**：在 agent 编排层，当同一轮内先调用了 `solo_record` 成功，后调用 `solo_health_record` 时，由系统把 record_id 回填到 health_record（best-effort，依赖编排层是否暴露同轮工具返回值）。若编排层暂不支持，则 record_id 留空。
3. **record_id 可空**：独立记录（如手动新增、未来导入）时 record_id 为空，属于正常状态。
4. **降级关联**：当 record_id 为空时，前端/统计仍可按 `date + subject` 软关联到当天日志，不影响健康页面的主体功能。

> 即：**record_id 是"尽力而为的强关联"，不是必需的**。这把决策表里"与 records 表可关联"这条理由修正为"可选关联"，避免过度承诺。
...
def _tool_health_record() -> ToolDefinition:
    return _definition(
        ...
        [
            ...
            # 注意：record_id 不暴露给 LLM，由系统侧回填（见 §2.6）
        ],
    )
```
...
| 存储位置 | 独立数据库 vs SoloStore 新表 | SoloStore 新表 | 复用现有连接/迁移机制、与 records 表可软关联（date+subject） |
...
| record_id 关联 | LLM 传值 vs 系统回填 vs 不关联 | **系统 best-effort 回填** | LLM 无法获知同轮 record id；系统侧尽力关联，失败则按 date+subject 软关联，不阻塞主流程 |
