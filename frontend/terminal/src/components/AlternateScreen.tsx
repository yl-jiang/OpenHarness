 ...
 * this component, enables SGR mouse tracking so wheel events are delivered to
 * the app, and ensures everything is restored when the component unmounts or
 * the process exits unexpectedly.
 */
export function AlternateScreen({children}: {children: React.ReactNode}): React.JSX.Element {
	const {stdout} = useStdout();

	useEffect(() => {
		if (!stdout) return;
		stdout.write(ENTER_ALT + ENTER_MOUSE + HIDE_CURSOR);
		const cleanup = (): void => {
			try {
