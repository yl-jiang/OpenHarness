...
router = APIRouter(prefix="/api/wolo", tags=["wolo"])
...
def _service(workspace: str | None = None) -> WoloService:
    key = workspace or "__default__"
    svc = _service_cache.get(key)
    if svc is None:
        svc = WoloService(workspace)
        _service_cache[key] = svc
    return svc


_service_cache: dict[str, WoloService] = {}
...
def decisions(
    ...
    return _service(workspace).list_decisions(project=project, query=query)
...
def highlights(
    ...
    return _service(workspace).list_highlights(kind=kind, project=project, query=query)
...
class WoloProjectCreateRequest(BaseModel):
    title: str
    description: str = ""
    priority: str = "medium"
    start_date: str = ""
...
class WoloProjectUpdateRequest(BaseModel):
    title: str | None = None
...
class WoloMilestoneCreateRequest(BaseModel):
    title: str
    description: str = ""
    target_date: str = ""


class WoloProjectLinkCreateRequest(BaseModel):
    entity_type: str
    entity_id: str
    source: str = "user"


class WoloArchiveRequest(BaseModel):
    reason: str = ""
...
def create_project(request: WoloProjectCreateRequest, workspace: str | None = None):
...
def update_project(project_id: str, request: WoloProjectUpdateRequest, workspace: str | None = None):
...
def archive_project(
    project_id: str,
    request: WoloArchiveRequest | None = None,
    workspace: str | None = None,
):
...
def create_milestone(project_id: str, request: WoloMilestoneCreateRequest, workspace: str | None = None):
...
def create_project_link(project_id: str, request: WoloProjectLinkCreateRequest, workspace: str | None = None):
