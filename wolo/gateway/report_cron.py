    ...
    *,
    ...
    if existing is not None:
        ...
        if updated:
            ...
            logger.info("Reconciled wolo report cron job %s", job_name)
        else:
            logger.debug("wolo report cron job already exists: %s", job_name)
        return
    ...
    logger.info(
        "Registered wolo report cron job: %s (schedule=%s tz=%s)",
        job_name,
        schedule,
        timezone_name,
    )
