...
from wolo.prompts import HEARTBEAT_EVAL_SYSTEM_PROMPT
...
class WoloHeartbeatService:
    """Signal-based periodic watchdog for the wolo app.
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "wolo heartbeat started interval_s=%d quiet=%s-%s tz=%s daily_cap=%d",
            self._interval_s,
        ...
        for todo in todos:
            if todo.due_date and todo.due_date <= today_str:
                signals.overdue_todos.append(
                    f"[{todo.priority}] {todo.project} {todo.title}（due: {todo.due_date}）"
                )

        # 3. Active blockers
        blockers = store.list_highlights(kind="blocker", limit=20)
        for item in blockers:
            if not self._is_recent(item.created_at, max_age_days=_BLOCKER_MAX_AGE_DAYS):
                continue
            signals.blockers.append(item.title)
    ...
    def _check_scheduler_down(self) -> bool:
        try:
            ...
            if not jobs_file.exists():
                return False
            from wolo.gateway.cron_scheduler import is_scheduler_running
    ...
    async def trigger_once(self) -> HeartbeatResult:
        ...
        if self._in_quiet_hours():
            logger.debug("wolo heartbeat skipped: quiet hours")
        ...
        if self._daily_cap_reached():
            logger.info(
                "wolo heartbeat skipped: daily cap reached (%d/%d)",
                self._pushes_today,
                self._max_daily_pushes,
            )
        ...
        signals = self.gather_signals()
        if signals.is_empty:
            logger.debug("wolo heartbeat skipped: no signals")
        ...
        if not unacked and not has_unacked_agent_task:
            logger.debug("wolo heartbeat skipped: all signals acked")
    ...
    async def _evaluate_signals(
        ...
        prompt = (
            "【heartbeat 自动触发】你是 wolo 的定时心跳 agent。\n"
