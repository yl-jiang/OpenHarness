"""Gateway service lifecycle for the standalone wolo app."""
...
class WoloGatewayService:
    """Foreground/background service wrapper for wolo channels."""
    ...
    async def run_foreground(self) -> int:
        ...
        if existing_pid is not None:
            logger.error(
                "wolo gateway already running pid=%d workspace=%s",
                existing_pid,
                self._workspace,
            )
            print(f"Error: wolo gateway is already running (pid {existing_pid})")
            return 1
        ...
        logger.info(
            "wolo gateway starting pid=%d channels=%s profile=%s workspace=%s",
            os.getpid(),
            channels,
        ...
        logger.info("wolo gateway ready — waiting for messages (Ctrl-C to stop)")

        try:
            await stop_event.wait()
        finally:
            logger.info("wolo gateway shutting down pid=%d", os.getpid())
            self._bridge.stop()
            bridge_task.cancel()
            manager_task.cancel()
...
def start_gateway_process(cwd: str | Path | None = None, workspace: str | Path | None = None) -> int:
    service = WoloGatewayService(cwd, workspace)
...
def gateway_status(cwd: str | Path | None = None, workspace: str | Path | None = None) -> WoloState:
    service = WoloGatewayService(cwd, workspace)
    ...
    if service.state_file.exists():
        with contextlib.suppress(Exception):
            last_error = WoloState.model_validate_json(
...
def _register_feed_digest_cron(
    ...
    """Best-effort registration of the wolo feed digest cron job."""
    fd_config = getattr(config, "feed_digest", None)
    if fd_config is None or not getattr(fd_config, "enabled", False):
        logger.debug("Wolo feed digest disabled; skipping cron registration")
        return
    try:
...
def _register_report_cron(workspace: str | Path | None) -> None:
    ...
    try:
        ...
        logger.warning("Failed to register wolo report cron jobs: %s", exc)
