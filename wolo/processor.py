...
class WoloProcessor:
        ...
        *,
        ...
        if backfill_missing_yesterday or process_date is not None:
            ...
            if not self.store.has_activity_on(yesterday):
                ...
                backfill_prompt = f"发现昨天（{yesterday}）没有工作记录。可以回复 `/wolo backfill {yesterday} 具体内容` 补录。"
    ...
    async def _generate_daily_question(self, process_date: str | None = None) -> str | None:
        """Generate a personalized work question based on the user's profile and context."""
    ...
    async def _persist_artifacts_for_record(
        ...
        persist_work_artifacts(self.store, record, artifacts)
    ...
    def _work_artifacts_context(self) -> str:
        sections: list[str] = []
        ...
        if todos:
            sections.append(
                "## Open Todos\n"
                + "\n".join(
                    f"- [{todo.priority}] {todo.title} project={todo.project} due={todo.due_date}"
                    for todo in todos
                )
            )
        decisions = self.store.list_decisions(limit=30)
        if decisions:
            sections.append(
                "## Decisions\n"
                + "\n".join(
                    f"- {decision.title} project={decision.project}; rationale={decision.rationale}; impact={decision.impact}"
                    for decision in decisions
                )
            )
        highlights = self.store.list_highlights(limit=50)
        if highlights:
            sections.append(
                "## Highlights\n"
                + "\n".join(
                    f"- [{item.kind}] {item.title} project={item.project}; {item.content}"
