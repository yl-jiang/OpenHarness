...
| 让你以后主动提醒某事（如“2分钟后提醒我喝水”） | → wolo_remind |
| 让你在未来某时间执行一个任务并把结果发给我（如"明天12点生成一份周报"） | → wolo_schedule |
...
- 对于一次性未来提醒，优先用 `wolo_remind`；若用户没说清提醒内容或未来时间，再用 `wolo_clarify`
- 对于需要在未来执行并返回结果的任务（如生成周报、整理记录），用 `wolo_schedule`
