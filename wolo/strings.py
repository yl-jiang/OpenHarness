...
HELP_TEXT = (
    "wolo 可以把工作输入整理成可查询、可复盘的工作记录。\n"
    "最省事的用法：直接发进展、会议结论、blocker、prompt/tool 经验或 PR/issue 摘要，我会自动入库并整理。\n"
    "常用命令：\n"
    "- /wolo process：整理待处理记录\n"
    "- /wolo view [数量]：查看最近记录，例如 /wolo view 20\n"
    "- /wolo report weekly|monthly|yearly：生成周/月/年报告\n"
    "- /wolo status：查看记录、待办与运行状态\n"
    "- /wolo llm-usage：查看模型调用统计\n"
    "- /wolo backfill [YYYY-MM-DD] 内容：补录过去的工作记录\n"
    "- 直接提问待办、blocker、决策或 prompt/tool 经验：查询相关工作沉淀"
)
