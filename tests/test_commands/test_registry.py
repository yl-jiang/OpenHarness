...
@pytest.mark.asyncio
async def test_commit_command_is_marked_local_only(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENHARNESS_CONFIG_DIR", str(tmp_path / "config"))
    registry = create_default_command_registry()
    command, _ = registry.lookup("/commit msg")
    assert command is not None
    assert command.remote_invocable is False


@pytest.mark.asyncio
async def test_commit_command_supports_explicit_remote_admin_opt_in(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENHARNESS_CONFIG_DIR", str(tmp_path / "config"))
    registry = create_default_command_registry()
    command, _ = registry.lookup("/commit msg")
    assert command is not None
