...
def test_dry_run_uses_preview_builder_and_skips_repl(monkeypatch):
    ...
    monkeypatch.setattr("openharness.cli_dry_run.build_dry_run_preview", fake_build_dry_run_preview)
    monkeypatch.setattr("openharness.ui.app.run_repl", fake_run_repl)
...
def test_dry_run_rejects_continue_resume(monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr("openharness.cli_dry_run.build_dry_run_preview", lambda **kwargs: {"mode": "dry-run"})
...
def test_cli_dry_run_module_exports_public_builders():
    import openharness.cli_dry_run as cli_dry_run

    assert callable(cli_dry_run.build_dry_run_preview)
    assert callable(cli_dry_run.format_dry_run_preview)
