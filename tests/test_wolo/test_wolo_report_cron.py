...
from wolo.core.workspace import ensure_workspace, get_data_dir
...
def test_wolo_report_cron_job_registration_and_idempotent(tmp_path: Path) -> None:
    from wolo.gateway.report_cron import ensure_report_jobs

    workspace = ensure_workspace(tmp_path / ".wolo")
    ensure_report_jobs("wolo", workspace=workspace)
    ensure_report_jobs("wolo", workspace=workspace)

    cron_path = get_data_dir(workspace) / "cron_jobs.json"
    jobs = json.loads(cron_path.read_text(encoding="utf-8"))
    weekly = [job for job in jobs if job["name"] == "wolo-weekly-report"]
    monthly = [job for job in jobs if job["name"] == "wolo-monthly-report"]
    ...
    assert "--job-name wolo-weekly-report" in weekly[0]["command"]
    assert "--report-type monthly" in monthly[0]["command"]


def test_wolo_report_cron_reconciles_existing_drift(tmp_path: Path) -> None:
    from wolo.gateway import report_cron
    ...
    report_cron.ensure_report_jobs("wolo", workspace=workspace)

    jobs = json.loads(jobs_path.read_text(encoding="utf-8"))
    weekly = next(job for job in jobs if job["name"] == "wolo-weekly-report")
    monthly = next(job for job in jobs if job["name"] == "wolo-monthly-report")
...
def test_wolo_report_runner_generates_calendar_week_from_due_job(tmp_path: Path, monkeypatch) -> None:
    from wolo.gateway.report_cron import ensure_report_jobs
    from wolo.gateway import report_runner
    ...
    monkeypatch.setattr("wolo.agent.OpenHarnessWoloAgent", lambda *args, **kwargs: object())
    pushed: list[str] = []
    ...
    assert asyncio.run(
        report_runner._main("wolo", "weekly", str(workspace), "wolo-weekly-report", "Asia/Shanghai")
    ) == 0

    reports = WoloStore(workspace).list_reports()
...
def test_wolo_report_runner_monthly_guard_and_timezone_conversion(tmp_path: Path, monkeypatch) -> None:
    from wolo.gateway.report_cron import ensure_report_jobs
    from wolo.gateway import report_runner
    ...
    assert asyncio.run(
        report_runner._main("wolo", "monthly", str(workspace), "wolo-monthly-report", "Asia/Shanghai")
    ) == 0
    assert WoloStore(workspace).list_reports() == []
    assert pushed == []
    ...
    assert asyncio.run(
        report_runner._main("wolo", "monthly", str(workspace), "wolo-monthly-report", "America/New_York")
    ) == 0
