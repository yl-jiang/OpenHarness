...
from openharness.memory import add_memory_entry, get_project_memory_dir, remove_memory_entry
...
def test_team_memory_remove_updates_team_index(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OPENHARNESS_DATA_DIR", str(tmp_path / "data"))
    project = tmp_path / "repo"
    project.mkdir()

    path = add_memory_entry(project, "Shared Rule", "Review before merging.", scope="team")

    assert remove_memory_entry(project, "shared_rule") is True
    assert "disabled: true" in path.read_text(encoding="utf-8")
    assert "shared_rule.md" not in (path.parent / "MEMORY.md").read_text(encoding="utf-8")
