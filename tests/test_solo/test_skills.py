"""Tests for solo default skills and workspace skill distribution."""
...
from solo.core.workspace import get_default_skills_dir, get_skills_dir, initialize_workspace
from solo.runner import _build_skill_guidance


DEFAULT_SKILL_NAMES = {
    "solo-health-recording",
    "solo-finance-recording",
    "solo-period-tracking",
    "solo-project-management",
    "solo-memory-extraction",
    "solo-todo-closure",
}
...
def test_initialize_workspace_preserves_existing_skills_and_copies_missing() -> None:
    with TemporaryDirectory() as tmp:
        ...
        # Existing custom skill should be preserved
        ...
        # An existing default skill that the user has modified should not be overwritten
        existing_health_dir = user_skills_dir / "solo-health-recording"
        existing_health_dir.mkdir()
        (existing_health_dir / "SKILL.md").write_text(
        ...
        assert "solo-health-recording" in copied
        # Missing default skills should still be copied
        assert "solo-project-management" in copied
        # The user-modified default skill should not have been overwritten
        assert (existing_health_dir / "SKILL.md").read_text(encoding="utf-8").startswith("---\nname: solo-health-recording\ndescription: user version")
