...
@pytest.mark.asyncio
async def test_solo_scheduler_runs_one_shot_reminder_and_removes_job(
    ...
    assert "站起来走走" in history[-1]["stdout"]
    ...
    assert "站起来走走" in send_dm.await_args.kwargs["content"]
    assert get_logs_dir(_workspace).exists()


@pytest.mark.asyncio
async def test_solo_scheduler_runs_agent_task_and_removes_job(
    monkeypatch: pytest.MonkeyPatch,
    _workspace: Path,
) -> None:
    ...
    with patch("solo.gateway.cron_scheduler._run_agent_task", new=AsyncMock(return_value="今天你记录了3条日志")):
        await scheduler.run_scheduler_loop(once=True)
    ...
    assert history[-1]["name"] == job["name"]
    assert history[-1]["status"] == "success"
    assert "今天你记录了3条日志" in history[-1]["stdout"]
    assert all(item["name"] != job["name"] for item in jobs)
    reformat.assert_not_awaited()
    send_dm.assert_awaited_once()
...
@pytest.mark.asyncio
async def test_solo_execute_job_uses_custom_timeout(
    monkeypatch: pytest.MonkeyPatch,
    _workspace: Path,
) -> None:
