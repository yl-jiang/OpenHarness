...
from ohmo.self_log import SelfLogStore
...
def test_gateway_router_ignores_thread_in_p2p():
    ...
    assert session_key_for_message(message) == "feishu:ou_abc"
...
def test_gateway_router_falls_back_to_chat_scope():
    ...
    # No chat_type metadata → defaults to private (not in shared set)
...
@pytest.mark.asyncio
async def test_gateway_bridge_records_self_log_without_runtime(tmp_path: Path):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    ...
    bridge = OhmoGatewayBridge(bus=bus, runtime_pool=FakeRuntimePool(), workspace=workspace)
    task = asyncio.create_task(bridge.run())
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="oc_log",
                content="/self-log 今天状态不错，晚上继续复盘",
                metadata={"chat_type": "group", "message_id": "om_1"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    finally:
    ...
    entries = SelfLogStore(workspace).list_entries()
    assert runtime_called is False
    assert outbound.content == "✅ 已记录到 self-log。"
    assert len(entries) == 1
    assert entries[0].content == "今天状态不错，晚上继续复盘"
    assert entries[0].channel == "feishu"
    assert entries[0].message_id == "om_1"


@pytest.mark.asyncio
async def test_gateway_bridge_self_log_bypasses_agent_group_reply_policy(tmp_path: Path):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    ...
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="oc_log",
                content="/self-log record 没有 @ohmo 也应该能记日志",
    ...
    entries = SelfLogStore(workspace).list_entries()
    assert outbound.content == "✅ 已记录到 self-log。"
    assert [entry.content for entry in entries] == ["没有 @ohmo 也应该能记日志"]


@pytest.mark.asyncio
async def test_gateway_bridge_records_bare_text_in_self_log_app_mode_without_runtime(tmp_path: Path, monkeypatch):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    runtime_called = False
    ...
    class FakeSemanticAgent:
        def __init__(self, store, router=None, agent_factory=None):
            self.store = store

        async def run(self, text):
            self.store.record(text)
            return "✅ 已记录到 self-log。"

    monkeypatch.setattr("ohmo.gateway.bridge.SelfLogToolAgent", FakeSemanticAgent)

    bridge = OhmoGatewayBridge(
        bus=bus,
        runtime_pool=FakeRuntimePool(),
        workspace=workspace,
        self_log_default_record=True,
    )
    task = asyncio.create_task(bridge.run())
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="oc_log",
                content="今天直接用 bot 记录，不输入斜杠命令",
                metadata={"chat_type": "p2p", "message_id": "om_bare"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    finally:
    ...
    assert runtime_called is False
    assert outbound.content == "✅ 已记录到 self-log。"
    assert [entry.content for entry in entries] == ["今天直接用 bot 记录，不输入斜杠命令"]


@pytest.mark.asyncio
async def test_gateway_bridge_routes_bare_self_log_text_through_semantic_tool_agent(
    tmp_path: Path,
    monkeypatch,
):
    workspace = tmp_path / ".ohmo-home"
    ...
    class FakeSemanticAgent:
        ...
        async def run(self, text):
            ...
            self.store.record("由语义工具记录")
            return "✅ semantic tool recorded"
    ...
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                chat_id="oc_log",
                content="帮我生成本周复盘",
                metadata={"chat_type": "p2p", "message_id": "om_semantic"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    finally:
    ...
    assert runtime_called is False
    assert semantic_calls == ["帮我生成本周复盘"]
    assert outbound.content == "✅ semantic tool recorded"
    assert SelfLogStore(workspace).list_entries()[0].content == "由语义工具记录"


@pytest.mark.asyncio
async def test_gateway_bridge_keeps_bare_text_on_normal_runtime_by_default(tmp_path: Path):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    runtime_called = False
    ...
    class FakeRuntimePool:
        async def stream_message(self, message, session_key):
            nonlocal runtime_called
            runtime_called = True
            yield SimpleNamespace(kind="final", text="normal reply", metadata={})
    ...
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="ou_user",
                content="普通 ohmo 私聊仍然进入 runtime",
                metadata={"chat_type": "p2p"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    ...
    assert outbound.content == "normal reply"
    assert SelfLogStore(workspace).list_entries() == []


@pytest.mark.asyncio
async def test_gateway_bridge_remote_self_log_report_uses_app_agent_without_runtime(tmp_path: Path, monkeypatch):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    SelfLogStore(workspace).record("今天完成了远程周报")
    bus = MessageBus()
    runtime_called = False
    ...
    class FakeAgent:
        ...
        async def generate_report(self, report_type, records, profile_context):
            ...
            assert records[0]["summary"] == "完成远程周报"
            return "## 本周概览\n- 手机 bot 已自动生成报告"

    monkeypatch.setattr("ohmo.gateway.bridge.OpenHarnessSelfLogAgent", lambda: FakeAgent())
    ...
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="oc_log",
                content="/self-log report weekly",
                metadata={"chat_type": "group", "message_id": "om_report"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    finally:
    ...
    assert runtime_called is False
    assert "手机 bot 已自动生成报告" in outbound.content
    assert SelfLogStore(workspace).list_reports()[0].report_type == "weekly"


@pytest.mark.asyncio
async def test_gateway_bridge_remote_self_log_help_without_runtime(tmp_path: Path):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    runtime_called = False
    ...
    assert runtime_called is False
    assert "/self-log report weekly" in outbound.content
    assert "/self-log process" in outbound.content


@pytest.mark.asyncio
async def test_gateway_bridge_remote_self_log_process_prompts_missing_backfill(tmp_path: Path, monkeypatch):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    ...
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="oc_log",
                content="/self-log process 2026-05-16",
                metadata={"chat_type": "group"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    ...
    assert "发现昨天（2026-05-15）没有记录" in outbound.content
    assert "/self-log backfill 2026-05-15" in outbound.content


@pytest.mark.asyncio
async def test_gateway_bridge_remote_self_log_backfill_processes_with_source(tmp_path: Path, monkeypatch):
    workspace = tmp_path / ".ohmo-home"
    initialize_workspace(workspace)
    bus = MessageBus()
    ...
    try:
        await bus.publish_inbound(
            InboundMessage(
                ...
                sender_id="ou_user",
                chat_id="oc_log",
                content="/self-log backfill 2026-05-15 昨天补录一个关键进展",
                metadata={"chat_type": "group"},
            )
        )
        outbound = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
    ...
    records = SelfLogStore(workspace).list_records()
    assert "backfilled: 2026-05-15" in outbound.content
...
@pytest.mark.asyncio
async def test_gateway_bridge_group_command_rejects_group_chat():
    bus = MessageBus()
