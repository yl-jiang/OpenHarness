...
_REPLY_MARKER = "\n[回复]"
_SOLO_DISABLE_HINT_DEDUPE = "SOLO_DISABLE_HINT_DEDUPE"
...
_PENDING_TODOS_HEADER = (
    "## Pending Todos",
    "",
    "The following todos are currently pending. "
    "When the user's message implies completion of any of these, "
    "call solo_done (or solo_update_todo with status=done) to mark it complete.",
    "",
)


def _build_pending_todos_context(store: SoloStore) -> str:
    ...
    for todo in todos:
        ...
        lines.append(f"- {todo.id}: {todo.title}{category}{due}")
    lines.append("")
    return "\n".join(lines)
...
def _build_active_projects_context(store: SoloStore) -> str:
    try:
...
class SoloQueryRunner:
    ...
    async def stream_run(
        ...
        *,
        ...
        previous_messages, session_id = ([], None)
        if session_key and use_session_history:
            previous_messages, session_id = load_conversation(workspace, session_key)
        ...
        if len(previous_messages) > _SESSION_MAX_MESSAGES:
            total_messages = len(previous_messages)
            previous_messages = trim_conversation_history_to_turn_boundary(
                previous_messages,
        ...
        if previous_messages:
            engine.load_messages(sanitize_conversation_messages(previous_messages))
        ...
        had_reasoning = False
        hint_dedupe_enabled = os.environ.get(_SOLO_DISABLE_HINT_DEDUPE) != "1"
        ...
        try:
            async for item in _process_stream(
                ...
                log_prefix="solo tool blocked or failed",
            ):
                yield item
        ...
        if (
            ...
            try:
                async for item in _process_stream(
                    ...
                    log_prefix="solo retry: tool blocked",
                ):
                    yield item
            except Exception as exc:
                engine_error = f"{type(exc).__name__}: {exc}"
        ...
        if passthrough_output and stream_finished_reason is None:
            ...
            logger.warning(
                "solo empty-response fallback — had_reasoning=True engine_error=%s session_key=%s text_preview=%r",
