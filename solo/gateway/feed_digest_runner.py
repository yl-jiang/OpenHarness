#!/usr/bin/env python3
"""Cron runner script for solo feed digest.

Called by the cron scheduler. Runs one feed digest cycle, archives the result,
and if --no-push is NOT set, pushes the archived digest to IM.
"""
...
async def _push_to_im(workspace: str | None, content: str) -> None:
    ...
    try:
        ...
        from solo.gateway.cron_scheduler import _send_feishu_dm
        ...
        logger.info("Pushed feed digest to feishu DM")
    except Exception as exc:
