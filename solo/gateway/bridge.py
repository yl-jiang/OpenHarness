...
from solo.core.models import ProjectLink, ProjectSuggestion
...
class SoloGatewayBridge:
    ...
    def _track_record_for_matching(self, store: SoloStore) -> None:
    ...
    async def _run_match_records(self, record_ids: list[str]) -> None:
        ...
        for rid in record_ids:
            try:
                ...
                # Solo uses category instead of project on todos
                artifact_projects = _collect_solo_artifact_projects(store, rid)
...
def _collect_solo_artifact_projects(store: SoloStore, record_id: str) -> list[str]:
    """Collect category strings from solo todos of a record."""
    projects: set[str] = set()
    for todo in store.list_todos():
        if todo.record_id == record_id and todo.category:
            projects.add(todo.category)
