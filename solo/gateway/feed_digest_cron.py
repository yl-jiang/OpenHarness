"""Auto-register feed digest cron jobs for the solo app."""
...
def ensure_feed_digest_job(
    ...
    *,
    ...
    """Register or update the feed digest cron job."""
    ...
    if existing is not None:
        ...
        if existing.get("command") != expected_command:
            ...
            changed = True
        if existing.get("timeout_s") != 1500:
            existing["timeout_s"] = 1500
            changed = True
        if changed:
            _upsert_cron_job(existing, workspace)
            logger.info("Updated feed digest cron job %s", job_name)
        else:
            logger.debug("feed digest cron job already exists: %s", job_name)
        return
    ...
    logger.info(
        "Registered feed digest cron job: %s (schedule=%s tz=%s)",
        job_name,
        schedule,
        tz,
    )
