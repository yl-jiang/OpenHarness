...
from solo.core.workspace import get_data_dir, get_sessions_dir
...
def _write_dream_snapshot(
    ...
    try:
        ...
        logger.warning("solo session: failed to write dream snapshot %s", path)
...
def save_conversation(
    ...
    try:
        ...
        if changed:
            logger.info(
                "solo session: dropped incomplete trailing turn session_key=%s kept=%d",
                session_key,
                len(clean),
            )
...
def load_conversation(
    ...
    try:
        ...
        if changed:
            ...
            conn.commit()
            logger.info(
                "solo session: healed incomplete trailing turn session_key=%s kept=%d",
                session_key,
                len(messages),
            )
        logger.debug(
