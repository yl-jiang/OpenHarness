...
def _resolve_image_generation_config(settings) -> dict[str, str]:
    ...
    try:
        ...
        logger.debug("codex image generation auth unavailable: %s", exc)
...
def _print_auth_resolution_error(settings, exc: Exception) -> None:
    ...
    if auth_source in {"claude_subscription", "codex_subscription"}:
        ...
        print(
            f"Error: {message}\n"
            "  This profile uses subscription auth, not an API key.\n"
...
async def build_runtime(
    *,
    ...
    # full_auto mode: register done tool (keep ask_user_question available)
...
async def handle_line(
    ...
    *,
    ...
    elif user_message is None and line.startswith("/"):
