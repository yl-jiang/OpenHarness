...
class AnthropicApiClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        ...
        if request.tools:
            params["tools"] = _normalize_tools_for_anthropic(request.tools)
        if self._claude_oauth:
        ...
        # Post-process: convert SDK message into internal representation.
...
def _normalize_tools_for_anthropic(tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Ensure tools use Anthropic's canonical format (input_schema, not parameters)."""
    result = []
    for tool in tools:
        if "input_schema" in tool:
            result.append(tool)
        elif "parameters" in tool:
            normalized = {k: v for k, v in tool.items() if k != "parameters"}
            normalized["input_schema"] = tool["parameters"]
