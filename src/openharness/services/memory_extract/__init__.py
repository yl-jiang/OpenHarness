...
def apply_extraction_records(cwd: str | Path, records: tuple[ExtractionRecord, ...]) -> ExtractionResult:
    ...
    for record in records:
        if record.scope == "team":
            ...
            if secret_error:
                logger.warning("memory extraction skipped team record %r: %s", record.title, secret_error)
                continue
            path, error = validate_team_memory_write_path(cwd, f"{record.title}.md")
            if error or path is None:
                logger.warning("memory extraction skipped team record %r: %s", record.title, error)
                continue
        written.append(
            add_memory_entry(
                cwd,
