...
async def _notify_job_result(job: dict[str, Any], entry: dict[str, Any]) -> None:
    ...
    except Exception as exc:  # noqa: BLE001 — notification failures must not break scheduler tick
...
async def execute_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    except Exception as exc:  # noqa: BLE001 — _command_for_job may surface config IO errors
        entry = {
            "name": name,
            "command": "",
