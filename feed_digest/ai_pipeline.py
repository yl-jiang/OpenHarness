...
def _completion_system_prompt(system_prompt: str, *, expected_json: str | None) -> str:
    if expected_json in {"object", "list"}:
        ...
        contract = (
            ...
            f"When your result is ready, call `feed_digest_emit_json` with a `payload` argument containing valid {expected_shape} content as a JSON string.\n"
            "IMPORTANT: Only call `feed_digest_emit_json` once you have the complete payload. "
            "Do NOT call it with empty or missing arguments as a probe or placeholder.\n"
            "Do not place the final JSON in normal assistant text.\n"
            "If the tool reports an error, read the error message carefully and retry with corrected arguments.\n"
        ...
        contract = (
            ...
            "IMPORTANT: Only call `feed_digest_emit_markdown` once you have the complete content ready.\n"
...
将评分结果通过工具输出（见下方 Output Contract）。"""
...
将去重结果通过工具输出（见下方 Output Contract）。"""
...
- 不要为了数量重复采集同一 query
- 规划好所有 actions 后，通过工具输出结果（见下方 Output Contract）
...
- 抽取完成后通过工具输出结果（见下方 Output Contract）
