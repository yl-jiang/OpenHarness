...
- `solo` / `wolo` router prompt now treats first-person future self-narrations ("周五要去..."、"下个月得...") as todo triggers instead of requiring an explicit "要做 X / 计划 X" phrasing; anti-patterns (others' plans, vague wishes like "想瘦一点") are still excluded. This closes a common miss where future-dated messages were only recorded but not turned into todos.
- `solo` router prompt now explicitly handles 生理期 / 经期 / 月经 / 大姨妈 messages: added `period` to the standard health category table, a dedicated "生理期记录操作细则" section with per-day record rules and field mapping, and examples routing the spouse's period to the spouse's database subject with `category="period"` and `metrics_json={"flow":..., "cycle_day":N}`. This fixes the miss where period-related Feishu messages were silently dropped from the health timeline.
- Onboard finance "理财盈亏趋势" chart now uses red for gains and green for losses (matching Chinese market convention) and draws both curves as solid lines, so the two series are distinguished by color rather than stroke style.
- Onboard finance trend x-axis labels now carry the month in `MM/DD` form (instead of month-only or day-only), across both "每日收支" (`CategoryRanking`) and "理财盈亏趋势" (`InvestTrend`). The underlying `/api/solo/finance/transactions/trend` endpoint was switched from monthly to daily bucketing so each data point has a full date key. Both cards adopt the Chinese-market palette — income/gain in red, expense/loss in green — with all curves drawn as solid lines.
